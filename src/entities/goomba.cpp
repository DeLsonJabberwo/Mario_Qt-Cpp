 ...
 *  in case needed */
flagEntity::flagEntity(float x, float y)
    : Enemy(x, y), m_isRight(false)
{
    createAnimation();
    //m_deathCounter = flagEntity_DEAD_TIMER;
    //m_speed        = flagEntity_SPEED;
}
...
void flagEntity::draw(GameScene &scene)
{
    QGraphicsPixmapItem* pItem = new QGraphicsPixmapItem();
    pItem->setZValue(GLOBAL::ENEMY_LAYER);
    pItem->setPos(int(position().x()), int(position().y()));
...
void flagEntity::update(float elapsedTime)
{
    m_velocityY += 0;
    m_velocityX = direction()*m_speed*elapsedTime;
    m_animator.update(elapsedTime);
...
void flagEntity::update(int cameraX, float elapsedTime)
{
    if( position().x() >= cameraX - GLOBAL::TILE_SIZE.width() - GLOBAL::ENTITY_UPDATE_AREA
        && position().x() < GLOBAL::ENTITY_UPDATE_AREA + GLOBAL::SCREEN_SIZE.width() + cameraX  )
    {
...
/*
void flagEntity::checkCollisionWithBlocks()
{
    for(int i = 0; i < Block::BLOCKS.size(); ++i)
    {
        collideWithBlock(Block::BLOCKS.at(i));
...
*/
/*
void flagEntity::collideWithBlock(Block *block)
{
    // Calculate potential new position
    float CollideX;
    float CollideY;
...
void flagEntity::checkCollisonWithEnemies()
{
    for(int i = 0; i < Enemy::ENEMIES.size(); ++i)
    {
        collideWithEnemy(Enemy::ENEMIES.at(i));
...
void flagEntity::collideWithEnemy(Enemy *enemy)
{
...
void flagEntity::createAnimation()
{
    m_texturePixmap.load(":/res/enemies_18x18.png");
    ...
    //0 1 2frames
    m_animator.m_mapStates["running"].push_back(m_texturePixmap.copy(0, TSH, TSW, TSH).scaled(18, 0));
    m_animator.m_mapStates["running"].push_back(m_texturePixmap.copy(TSW, TSH, TSW, TSH).scaled(18, 0));
/*
    m_animator.m_mapStates["running_f_"].push_back(m_texturePixmap.copy(3, TSH, TSW, TSH).scaled(TW, TH).transformed(QTransform().scale(-1,1)));
    m_animator.m_mapStates["running_f_"].push_back(m_texturePixmap.copy(TSW, TSH, TSW, TSH).scaled(TW, TH).transformed(QTransform().scale(-1,1)));
...
void flagEntity::setAnimation(QString state)
{
    if(m_isRight)
    {
        m_animator.changeState(state);
...
QRect flagEntity::hitBox()
{
    return QRect(position().x(), position().y(), GLOBAL::TILE_SIZE.width(), GLOBAL::TILE_SIZE.height());
}
void flagEntity::setDirection(bool isRight)
{
    m_isRight = isRight;
}
int flagEntity::direction() const
{
    if(m_isRight)
    {
        return 1;
